...
- 外部投稿者が自分のforkへ保存する場合、そのユーザー側にもAppをインストールする

Cloudflareへ`GITHUB_CLIENT_ID`と`GITHUB_CLIENT_SECRET`をSecretとして登録します。ローカルでは`.dev.vars.example`を`.dev.vars`へコピーして値を設定します。

## Cloudflare

1. `wrangler kv namespace create SESSIONS`と`wrangler kv namespace create SESSIONS --preview`を実行する。
2. 返されたIDを`wrangler.jsonc`の`SESSIONS`へ設定する。
